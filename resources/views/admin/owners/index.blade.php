                            ...
                            @foreach ($owners as $owner)
                              <tr>
                                ...
                                <td class="md:px-4 py-3">
                                  <button type="button" onclick="location.href='{{ route('admin.owners.edit', ['owner' => $owner->id]) }}'" class="bg-blue-200 border-0 py-2 px-4 focus:outline-none hover:bg-blue-400 rounded">編集</button>
                                </td>
                                <td class="md:px-4 py-3">
                                  <form id="delete_{{ $owner->id }}" method="POST" action="{{ route('admin.owners.destroy',
                                    ['owner' => $owner->id]) }}" >
                                    @method('delete')
                                    @csrf
                                    <a href="#" data-id="{{ $owner->id }}" onclick="deletePost(this)" class="bg-red-200 border-0 py-2 px-4 focus:outline-none hover:bg-red-400 rounded">削除</a>
                                  </form>
                                </td>
                              </tr>
                            @endforeach
